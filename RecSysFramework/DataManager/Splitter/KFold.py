...
class KFold(DataSplitter):
    ...
    def __init__(self, n_folds=5, forbid_new_split=False, force_new_split=False,
                 allow_cold_users=False, random_seed=42):
        """
    ...
    def load_split(self, datareader, save_folder_path=None, postprocessings=None, filename_suffix=""):
        ...
        if save_folder_path is None:
            save_folder_path = datareader.get_complete_default_save_path(postprocessings)
        ...
        try:
            ...
            for i in range(self.n_folds):
                for d in datalist:
                    if not datareader.all_files_available(save_folder_path + self.get_name() + os.sep,
            ...
            for i in range(self.n_folds):
                ...
                for d in datalist:
                    urm, urm_mappers, icm, icm_mappers, ucm, ucm_mappers = datareader.load_from_saved_sparse_matrix(
                        save_folder_path + self.get_name() + os.sep,
                        filename_suffix="{}_{}_{}".format(filename_suffix, i, d))
    ...
    def save_split(self, split, save_folder_path=None, filename_suffix="", split_number=None):
        ...
        super(KFold, self).save_split(split, save_folder_path=save_folder_path,
...
class WarmItemsKFold(KFold):
    ...
    def get_name(self):
        return "warm_items_{}_fold{}".format(self.n_folds, "" if self.allow_cold_users else "_no_cold_users")
    ...
    def split(self, dataset):
        ...
        for i in range(self.n_folds):
            ...
            # Assume that all the URMs have the same non-zero cells
            for URM_name in dataset.get_URM_names():
                ...
                urm[URM_name] = sps.csr_matrix((np.array(data)[mask], (np.array(row)[mask], np.array(col)[mask])), shape=URM.shape)
...
class ColdItemsKFold(KFold):
    ...
    def get_name(self):
        return "cold_items_{}_fold{}".format(self.n_folds, "" if self.allow_cold_users else "_no_cold_users")
