...
class DataSplitter(object):
    ...
    def load_split(self, datareader, save_folder_path=None, postprocessings=None, filename_suffix=""):
        ...
        if save_folder_path is None:
            save_folder_path = datareader.get_complete_default_save_path(postprocessings) + self.get_name() + os.sep

        try:

            datalist = self._get_dataset_names_in_split()
            for i in datalist:
                if not datareader.all_files_available(save_folder_path,
            ...
            for i in datalist:
                urm, urm_mappers, icm, icm_mappers, ucm, ucm_mappers = datareader.load_from_saved_sparse_matrix(
                    save_folder_path, filename_suffix="{}_{}".format(filename_suffix, i))
                datasets.append(Dataset(datareader.get_dataset_name(), base_folder=datareader.get_default_save_path(),
