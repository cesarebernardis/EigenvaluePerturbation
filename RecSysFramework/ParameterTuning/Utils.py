...
from RecSysFramework.ParameterTuning import SearchBayesianSkoptEigenPerturbation
...
def run_parameter_search(recommender_class, split_name, dataset_train, dataset_validation, dataset_test=None,
                         URM_name="URM_all", ICM_name="ICM_all", is_eigenperturbation=False,
                         output_folder_path=None, save_model="best",
    ...
    if isinstance(dataset_train, list):
        ...
        assert len(dataset_train) == len(dataset_validation), "Train and Validation dataset lists have different lengths"
    ...
    try:
        ...
        if is_eigenperturbation:
            parameterSearch = SearchBayesianSkoptEigenPerturbation(recommender_class,
        ...
        if recommender_class is FunkSVD:
            ...
            recommender_input_args = SearchInputRecommenderArgs(
                ...
                CONSTRUCTOR_KEYWORD_ARGS={},
                FIT_POSITIONAL_ARGS=fixed_positional_args,
                FIT_KEYWORD_ARGS=dict(earlystopping_keywargs, **fixed_keyword_args)
            )
